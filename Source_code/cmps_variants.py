...
def CMPS_v2(ori_mdata,mr_mdata,budget):
    ...
    for index in range(len(img_paths)):
        ...
        image_uncertainty[img_paths[index]] = w_gini * deepgini(predictions[index]) + w_maxp * maxp(predictions[index])
    ...
    if dataset =='fashion':
        ...
        for model in ['lenet1','lenet5']:
            ...
            for budget in [500,1000]:
                print(budget)
                print("CMPS++(-I1)")
                CMPS_v1(ori_mdata, mr_mdata, budget)
                print("CMPS++(-I2)")
                CMPS_v2(ori_mdata, mr_mdata, budget)
